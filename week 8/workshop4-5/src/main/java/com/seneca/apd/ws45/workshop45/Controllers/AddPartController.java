...
public class AddPartController {
    ...
    private void initialize() {
        idField.setText(String.valueOf(partIdCounter)); // Auto-generate ID
        idField.setEditable(false); // Prevent editing ID

        // Set event listeners for toggling fields dynamically
        inHouseRadio.setOnAction(e -> switchToInHouse());
        outsourcedRadio.setOnAction(e -> switchToOutsourced());

        // Default state: Show In-House fields, hide Outsourced fields
        switchToInHouse();
    }
    ...
    private void handleSave() {
        try {
            int id = partIdCounter++; // Auto-increment ID
