...
public class ListOfBookings {
    ...
    private void filterBookings(String keyword) {
        ...
        for (BookingView view : bookings) {
            if (view.getName().toLowerCase().contains(keyword.toLowerCase()) ||
                    String.valueOf(view.getBookingId()).contains(keyword)) {
                filtered.add(view);
            }
        }
    ...
    void handleUpdate(ActionEvent event) {
        ...
        ListOfBookings.BookingView selected = guestTable.getSelectionModel().getSelectedItem();

        if (selected != null) {
            try (Connection conn = HotelDatabase.connect()) {
                String query = """
                SELECT g.name, g.phone, g.email, g.address, rm.room_type, rm.numberOfBeds, rm.price, rm.status,
            ...
            """;
                ...
                if (rs.next()) {
                    ...
                    // Set in AppState
                    AppState.setSelectedGuest(guest);
                    AppState.setSelectedRoom(room);
                    AppState.setCheckInDate(LocalDate.parse(rs.getString("check_in_date")));
                    AppState.setCheckOutDate(LocalDate.parse(rs.getString("check_out_date")));
                    AppState.setSelectedBookingId(selected.getBookingId());

                    //  Go to UpdateBooking.fxml screen
                    SceneNavigator.switchTo((Stage) guestTable.getScene().getWindow(),
                ...
                e.printStackTrace();
                showWarning("Failed to fetch booking details.");
            }
        } else {
            showWarning("Please select a reservation to update.");
        }
    }
